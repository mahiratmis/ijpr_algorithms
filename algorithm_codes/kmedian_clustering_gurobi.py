...
#%%
def Improved_Kmedian(serviceRates, failureRates, NumberofCluster, holding_costs, similarityType=1):
    '''
    input: -service rates for each SKU
           -faliure rates for each SKU
           -number of cluster: between 1,2,..., number of SKUs
           -holding costs for each SKU
           -similarity Type---> all atributes have same weight (1)
                                service rate has higher weight  (2)
                                failure rate has higher weight  (3)
                                holding cost rate has higher weight  (4)


    output: -Clusters and SKUs in each cluster under different weights of attributes

    Uses IP formulation and solver to find cluster.  
    ...
    # cost matrix set up
    if similarityType == 1: #equal weight; i.e., 1.0/3.0
        
        #print "a"
        weight_service, weight_failure, weight_holding= 1.0/3.0, 1.0/3.0, 1.0/3.0
        
        size = (len(serviceRates), len(serviceRates))
        Cij2 = np.zeros(size)
        for i in range(len(serviceRates)):
            for j in range(i + 1, len(serviceRates)):
                Cij2[i, j] = Cij2[j, i] = weight_service*(serviceRates[i] - serviceRates[j]) ** 2+\
                weight_failure*(failureRates[i] - failureRates[j]) ** 2+\
                weight_holding*(holding_costs[i] - holding_costs[j]) ** 2
    ...
    if similarityType == 2:
        ...
        weight_service, weight_failure, weight_holding= 1, 0, 0
    ...
    if similarityType == 3: 
        ...
        #print "c"
        weight_service, weight_failure, weight_holding= 0, 1, 0
    ...
    if similarityType == 4: 
        ...
        weight_service, weight_failure, weight_holding= 0, 0, 1
...
#%%
def Improved_KmedianHeuristic(FailureRates, ServiceRates, holding_costs, penalty_cost, skillCost, machineCost, similarityType):
    '''
    ...
    for NumberofCluster in range(1, len(FailureRates) + 1):
        cluster_KMedian = Improved_Kmedian(ServiceRates, FailureRates, NumberofCluster, holding_costs, similarityType)
...
for case in json_case:
    if case["caseID"] != "Case: 000x":
        ...
        #for similarityType in [1]:
        for similarityType in [1, 2,3,4]:
            start_time = time.time()

            bestCost, bestHolding, bestPenalty, bestMachineCost, bestSkillCost, bestCluster, bestS, bestEBO, \
            bestserverAssignment, LogFileList = Improved_KmedianHeuristic(np.array(failure_rates), np.array(service_rates), \
                                                                 np.array(holding_costs), penalty_cost, skill_cost,
                                                                 machine_cost, similarityType)
            stop_time = time.time() - start_time

            print (case["caseID"], similarityType, bestCost)
...
with open('Improved_KmedianAll.json', 'w') as outfile:
